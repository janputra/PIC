...
{
    if (data & 0b1000) RA6=1;
    else RA6=0;
    if (data & 0b0100) RA7=1;
    else RA7=0;
    if (data & 0b0010) RA5=1;
    else RA5=0;
    if (data & 0b0001) RA4=1;
    else RA4=0;
    RA3=1;
...
void lcd_init(void)
{
     _delay((unsigned long)((30000)*(8000000/4000000.0)));

    data_write(0x03);
    _delay((unsigned long)((7000)*(8000000/4000000.0)));
    ...
    data_write(0x03);
    _delay((unsigned long)((850)*(8000000/4000000.0)));
...
void lcd_update(char cmd)
{
    ...
    switch (cmd)
    {
        ...
        case 'f':
              ...
              diff=fabsf(last_frequency-frequency);
...
void interrupt_enable(_Bool state)
{
    if (state)
    {
          INTE =1;
...
void pwm_con(void)
{
    if (pwmcon_flag)
    {
        ...
        if (last_pwm_lvl!=pwm_lvl)
        {
            lcd_update('p');
            dutycycle=(pwm_lvl*10);

            DC2B0=dutycycle & 0x01;
            DC2B1=dutycycle & 0x02;
            CCPR2L=dutycycle>>2&0xFF;

            last_pwm_lvl=pwm_lvl;
            read_volt();
